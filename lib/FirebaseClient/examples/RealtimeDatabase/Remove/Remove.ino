...
void loop()
{
    ...
    if (app.ready() && !taskComplete)
    {
        ...
        Database.set<String>(aClient, "/examples/Remove/data1", "Orange");
        Database.set<String>(aClient, "/examples/Remove/data2", "Banana");
        Database.set<String>(aClient, "/examples/Remove/data3", "Apple");

        Serial.println("Removing data... ");

        // Async call with callback function.
        Database.remove(aClient, "/examples/Remove/data1", processData, "removeTask");

        // Async call with AsyncResult for returning result.
        Database.remove(aClient, "/examples/Remove/data2", databaseResult);

        // Sync call which waits until the payload was received.
        bool status = Database.remove(aClient, "/examples/Remove/data3");
        if (status)
            Serial.println("Remove data (await) complete!");
        else
